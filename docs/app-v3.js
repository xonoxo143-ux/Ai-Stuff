...
const BLANK_OUTPUT_MARKER = "[Blank output: no text returned by model]";
...
const el = {
  ...
  promptFormatSelect: document.querySelector("#promptFormatSelect"),
...
function getSelectedModelId() {
  if (el.modelSelect.value === "custom") return el.customModelInput.value.trim();
...
function selectedPromptFormat() {
  return el.promptFormatSelect?.value || "auto";
}
...
function buildPlainInstruction(prompt) {
  const system = el.systemPromptInput.value.trim() || "You are a concise coding assistant.";
  return `${system}\n\nTask:\n${prompt}\n\nAnswer:`;
}
...
async function loadModel() {
  ...
  try {
    ...
    if (device === "webgpu") logStatus("Try Runtime → WASM, or use the 135M model first.");
  }
}
...
function stripPromptPrefix(text, promptInput) {
  if (typeof text !== "string") return "";
  const trimmed = text.trim();
  if (typeof promptInput === "string" && trimmed.startsWith(promptInput.trim())) {
    return trimmed.slice(promptInput.trim().length).trim();
  }
  return trimmed;
}

function extractGeneratedText(result, promptInput) {
  if (Array.isArray(result) && result[0]) {
    ...
    if (typeof item.generated_text === "string") return stripPromptPrefix(item.generated_text, promptInput);
    return JSON.stringify(item, null, 2);
  }
  if (typeof result === "string") return stripPromptPrefix(result, promptInput);
  return String(result ?? "");
}

async function generateOnce(prompt, format, generationOptions, useStreamer = true) {
  let streamedText = "";
  const modelInput = format === "plain"
    ? buildPlainInstruction(prompt)
    : format === "raw"
      ? buildRawPrompt(prompt)
  ...
  if (useStreamer) {
    options.streamer = new TextStreamer(state.generator.tokenizer, {
      skip_prompt: true,
  ...
  const result = await state.generator(modelInput, options);
  return {
    format,
    streamed: useStreamer,
    text: (streamedText.trim() || extractGeneratedText(result, modelInput).trim()),
    rawResult: result,
  };
}
...
async function runPrompt() {
  ...
  const maxNewTokens = Number(el.maxTokensInput.value || 128);
  const temperature = Number(el.temperatureInput.value || 0);
  const topP = Number(el.topPInput.value || 0.9);
  const promptFormat = selectedPromptFormat();
  const primaryFormat = promptFormat === "auto" ? "chat" : promptFormat;
  const promptItem = state.prompts.find((item) => item.id === el.promptSelect.value);
  const startTime = Date.now();
  let retryUsed = false;
  let blankOutput = false;
  let fallbackPath = primaryFormat;
  ...
  try {
    logStatus(`Generating with ${primaryFormat} prompt format.`);
    let generated = await generateOnce(prompt, primaryFormat, generationOptions, true);
    let finalText = generated.text.trim();
    let finalFormat = `${generated.format}${generated.streamed ? "/stream" : "/raw"}`;

    if (!finalText && primaryFormat !== "plain") {
      retryUsed = true;
      fallbackPath += " → plain/stream";
      logStatus("Blank output detected. Retrying once with plain instruction format.");
      el.outputBox.textContent = "";
      generated = await generateOnce(prompt, "plain", generationOptions, true);
      finalText = generated.text.trim();
      finalFormat = `${generated.format}${generated.streamed ? "/stream" : "/raw"}`;
    }

    if (!finalText) {
      retryUsed = true;
      fallbackPath += " → raw/no-stream";
      logStatus("Still blank. Retrying once with raw non-streaming generation.");
      el.outputBox.textContent = "";
      generated = await generateOnce(prompt, "raw", generationOptions, false);
      finalText = generated.text.trim();
      finalFormat = `${generated.format}${generated.streamed ? "/stream" : "/raw"}`;
    }
    ...
    if (!finalText) {
      blankOutput = true;
      finalText = BLANK_OUTPUT_MARKER;
      logStatus("Blank output persisted after all fallback paths.");
    }

    const durationMs = Date.now() - startTime;
    el.outputBox.textContent = finalText;
    el.resultMeta.textContent = `${state.modelId} · ${state.backend.toUpperCase()} · ${promptItem?.title ?? "Custom"} · ${finalFormat}${retryUsed ? " retry" : ""} · ${(durationMs / 1000).toFixed(1)}s`;
    state.lastRun = {
      ...
      output: finalText,
      durationMs,
      settings: { maxNewTokens, temperature, topP, promptFormat: finalFormat, retryUsed, blankOutput, fallbackPath },
      rating: blankOutput ? "Invalid / Blank" : "Unrated",
    };
    setStatus("Ready", "ready");
    logStatus(blankOutput ? "Generation complete with blank-output warning." : "Generation complete.");
  } catch (error) {
    setStatus("Error", "error");
...
function saveResult() {
  ...
  const entry = { ...state.lastRun, rating: state.lastRun.rating === "Invalid / Blank" ? "Invalid / Blank" : getRating() };
