...
function extractGeneratedText(result, promptInput) {
  if (Array.isArray(result) && result[0]) {
    ...
    if (typeof item.generated_text === "string") {
      ...
      if (typeof promptInput === "string" && text.startsWith(promptInput)) {
        return text.slice(promptInput.length).trim();
      }
      return text;
    }
  ...
  if (typeof result === "string") {
    if (typeof promptInput === "string" && result.startsWith(promptInput)) {
      return result.slice(promptInput.length).trim();
    }
    return result;
  }
...
async function generateOnce(prompt, format, generationOptions) {
  let streamedText = "";
  const modelInput = format === "plain" ? buildPlainInstruction(prompt) : buildChatMessages(prompt);
  ...
  const result = await state.generator(modelInput, {
    ...generationOptions,
    streamer,
...
async function runPrompt() {
  ...
  try {
    ...
    let generated = await generateOnce(prompt, primaryFormat, generationOptions);
    let finalText = generated.text.trim();
    let finalFormat = generated.format;

    if (!finalText && primaryFormat !== "plain") {
      retryUsed = true;
      logStatus("Blank output detected. Retrying once with plain instruction format.");
      el.outputBox.textContent = "";
      generated = await generateOnce(prompt, "plain", generationOptions);
      finalText = generated.text.trim();
      finalFormat = generated.format;
    }

    if (!finalText) {
      blankOutput = true;
      finalText = BLANK_OUTPUT_MARKER;
      logStatus("Blank output persisted after retry.");
    }
    ...
    state.lastRun = {
      ...
      output: finalText,
      durationMs,
      settings: { maxNewTokens, temperature, topP, promptFormat: finalFormat, retryUsed, blankOutput },
