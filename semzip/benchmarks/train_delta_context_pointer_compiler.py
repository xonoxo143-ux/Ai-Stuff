...
def main() -> None:
    ...
    # Keep the original baseline distribution exactly fixed.
    ...
    if not getattr(tokenizer, "is_fast", False):
        raise RuntimeError("context pointer benchmark requires tokenizer offsets")
    ...
    class ContextPointerDeltaCompiler(nn.Module):
        def __init__(self):
            super().__init__()
            self.backbone = backbone
            # Entity identity/context is compressed to a tiny pointer space.
            self.entity_projection = nn.Linear(hidden, d, bias=False)
            # The final causal state has seen the entire sentence and produces one
            # query per semantic pointer role.
        ...
        def forward(self, input_ids, attention_mask, entity_positions, entity_present):
            ...
            entity_states = states[
                torch.arange(batch).unsqueeze(1), safe_positions
            ]
            entity_vectors = self.entity_projection(entity_states)  # [B, 4, d]
            role_vectors = self.role_queries(pooled).view(batch, 6, d)  # [B, 6, d]
            ...
            none = self.none_head(pooled).unsqueeze(-1)
            pointer_logits = torch.cat((logits, none), dim=-1)
            return pointer_logits, self.bit_head(pooled)

    model = ContextPointerDeltaCompiler()
    ...
    report = {
        "model": args.model,
        "mode": "full_context_low_rank_entity_pointer_delta",
        "pointer_dim": d,
    ...
    Path("compiler-delta-context-pointer-results.json").write_text(json.dumps(report, indent=2))
    Path("semvm-delta-context-pointer-adapter").mkdir(exist_ok=True)
    model.backbone.save_pretrained("semvm-delta-context-pointer-adapter")
    torch.save({
        ...
        "hidden_size": hidden, "pointer_dim": d,
    }, "semvm-delta-context-pointer-adapter/pointer_heads.pt")

    print(f"model={args.model} total_parameters={total_parameters} trainable_parameters={trainable_parameters}")
    print(json.dumps({k: report[k] for k in (
        "mode", "pointer_dim", "train_samples", "eval_samples", "epochs",
        "trainable_parameters", "pointer_accuracy", "bit_accuracy",
        "exact_frame", "exact_frame_accuracy", "family_exact", "loss_history"
    )}, indent=2))
