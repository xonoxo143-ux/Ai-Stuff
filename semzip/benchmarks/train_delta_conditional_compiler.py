...
def main() -> None:
    ...
    class ConditionalDeltaCompiler(nn.Module):
        def __init__(self):
            super().__init__()
        ...
        def forward(self, input_ids, attention_mask):
            ...
            return (
                self.pointer_head(pooled).view(-1, 6, MAX_SLOTS + 1),
                self.bit_head(pooled),
            )

    model = ConditionalDeltaCompiler()
    ...
    def encode(group: list[DeltaExample]):
        encoded = tokenizer(
            [x.text for x in group], padding=True, truncation=True,
            max_length=96, return_tensors="pt",
        )
        pointers, bits = zip(*(frame_targets(x) for x in group))
        return encoded, torch.tensor(pointers, dtype=torch.long), torch.tensor(bits, dtype=torch.float32)
    ...
    for epoch in range(args.epochs):
        ...
        for group in batches(train, args.batch_size, rng):
            ...
            # Primary delta is always present.
            for position in range(3):
                loss = loss + pointer_loss(pointer_logits[:, position, :], pointers[:, position])
            # Secondary pointers are meaningful only on examples containing a second delta.
    ...
    with torch.no_grad():
        for start in range(0, len(evaluation), args.batch_size):
            ...
            # Conditional decode: absent secondary structure is supplied deterministically.
    ...
    report = {
        "model": args.model,
        "mode": "grounded_world_delta_conditional",
        ...
        "loss_history": loss_history, "mistakes": mistakes,
        "contract": {
            "secondary_pointer_loss_masked_when_absent": True,
            "secondary_payload_deterministic_when_absent": True,
            "free_form_generation": False,
            "lexical_action_classes": False,
        },
    }
    Path("compiler-delta-conditional-results.json").write_text(json.dumps(report, indent=2))
    Path("semvm-delta-conditional-adapter").mkdir(exist_ok=True)
    model.backbone.save_pretrained("semvm-delta-conditional-adapter")
    torch.save({
        "pointer_head": model.pointer_head.state_dict(),
        "bit_head": model.bit_head.state_dict(),
        "hidden_size": hidden,
    }, "semvm-delta-conditional-adapter/delta_heads.pt")
